"""Unit tests for Groq extraction provider."""
...
def _groq_response(content: object) -> dict[str, object]:
    return {"choices": [{"message": {"content": content}}]}


class TestGroqExtractionProvider:
    """Tests for GroqExtractionProvider."""
    ...
    def test_extract_content_supports_string(self):
        ...
        client = MagicMock()
        provider = GroqExtractionProvider(client=client)

        content = provider._extract_content(_groq_response('{"ok":true}'))
    ...
    def test_extract_content_supports_array_parts(self):
        ...
        content = provider._extract_content(
            _groq_response([{"text": "part-1"}, {"text": "part-2"}])
        )
    ...
    def test_run_ocr_uses_complete_with_pdf(self):
        ...
        client.complete_with_pdf = AsyncMock(
            return_value=_groq_response('{"pages":[{"page":1,"text":"abc"}]}')
        )
        provider = GroqExtractionProvider(client=client)
    ...
    def test_run_statement_uses_complete_with_text(self):
        ...
        client.complete_with_text = AsyncMock(
            return_value=_groq_response('{"statement_id":"x"}')
        )
        provider = GroqExtractionProvider(client=client)
